...
const eventDefinitions: Record<WorldEventType, EventDefinition> = {
  bandit_ambush: {
    type: 'bandit_ambush',
...
function maybeTriggerTimedEvent(state: GameState, dt: number): void {
  ...
  const cycle: WorldEventType[] = state.world.time.phase === 'night' ? ['crypt_spill', 'storm', 'rare_ore'] : ['merchant_caravan', 'market_day', 'bandit_ambush', 'lost_traveler'];
