...
export class UIManager {
  ...
  getPerformanceStats(): Pick<GameState['dev']['renderStats'], 'domNodeCount' | 'visibleWindowCount' | 'iconRenderRequestCount' | 'cachedIconCount' | 'eventListenerCount'> {
    ...
    return {
      ...
      visibleWindowCount: this.root.querySelectorAll('[data-window-id], .react-ui-layer [data-ui-window="true"]').length,
  ...
  private bindEvents(): void {
    ...
    this.root.addEventListener('click', (event) => {
      ...
      if (!button) return;
      if (target.closest('.react-ui-layer [data-react-panel]')) return;
