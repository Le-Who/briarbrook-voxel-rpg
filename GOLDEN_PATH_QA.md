...
| 11. Take/check one work order | Market board or work-order surface shows a concrete demand and reward, including Mira's first-hour log order. | If logs are not ready, chop one starter tree before choosing the order. | If board is hidden behind another window, reset layout and record layering risk. | Reload with market state and confirm orders remain sane. |
...
| 19. Turn in or complete one work order | Mira's Road Kit Timber order accepts six logs from the early gathering route and produces gold/reward plus a transaction log. | If logs were banked, complete the order from town or bank access. | If delivery consumes items without reward, block release and inspect EconomySystem. | Reload after completion and confirm gold, telemetry, and transaction log. |
...
- Automated coverage now guarantees a low-tier first-hour log work order after one starter tree harvest; the full browser Golden Path still needs to verify the route in live UI.
